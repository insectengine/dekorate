...
public interface ThorntailConfigHolder extends WithProject {
  ...
  default Map<String, Object> getThorntailConfig() {
    if (thorntailConfig.get() == null) {
      ...
      try {
        fileInputStream = new FileInputStream(getProject().getBuildInfo().getResourceDir().resolve("project-defaults.yml").toFile());

      properties.putAll(Maps.parseResourceFile(fileInputStream,"project-defaults.yml"));
